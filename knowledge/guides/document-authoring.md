...
5. **랩 스크립트 계약:** `labs.js`는 IIFE, 문서 내 요소 id 또는 문서 스코프
   ...
   부분은 자동 존중된다. 컨트롤 조작 결과를 요약하는 리드아웃(`.lab-readout`)에는
   `role="status"`를 단다 — 보조기술이 값 갱신을 알리게 하는 계약이다. 기존
   발행 문서들은 `aria-live="polite"`만 갖고 있었고(PCM 포함), 2026-08-06에
   `role="status"`를 전 문서 요건으로 승격했다. 기존 `aria-live`는 유지해도 된다.
6. **컨트롤은 그림을 실제로 바꿔야 한다.** 드롭다운·슬라이더가 캡션이나 숫자
   한 토큰만 바꾸고 도해 구조가 그대로면 "무엇이 달라지는가"가 전달되지 않는다
   (격리 문서 실결함, 2026-08-06 fable 검증). 상태 분기가 있는 랩은 분기마다
   달라지는 요소를 실제로 다시 그리고, 바뀐 요소에는 일시적 강조(플래시·감쇠·
   주석 밴드)를 준다. "판정 텍스트는 방지라는데 그림은 성공하는 모습" 같은
   판정-도해 모순은 출고 차단 사유다.
7. **코드블럭 계약:** `<pre class="code-block" data-lang="js|sql|python|c|bash"><code>…`
   형태로 쓴다. 하이라이팅·언어 라벨·복사 버튼은 `site.js`가 자동 처리한다
   (외부 하이라이터 금지 — 정적 사이트 계약). `data-lang`을 생략하면 무장식
   코드로 남으므로, 언어가 있는 코드는 반드시 명시한다. 가로 스크롤 표면
   (`.svg-scroll`, `.table-wrap`)의 스크롤 힌트도 `site.js` 자동이다.
